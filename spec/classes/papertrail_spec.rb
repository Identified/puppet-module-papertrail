...
describe 'papertrail', :type => :class do
  ...
  #  content = catalogue.resource('file', '/etc/rsyslog.d/papertrail.conf').send(:parameters)[:content]
  #
  #  content.should match(/^\$DefaultNetstreamDriverCAFile \/etc\/syslog.papertrail.crt # trust these CAs$/)
  #  content.should match(/^\$ActionSendStreamDriver gtls # use gtls netstream driver$/)
